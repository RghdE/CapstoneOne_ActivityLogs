...
class device_type(MRJob):

    # define the number of steps? (similir to the pipline) 
    # when I removed this function, the code didnt work for me
    def steps(self):
        return [
            MRStep(mapper=self.mapper_get_device,
                   reducer=self.reducer_sum_device),
            MRStep(reducer=self.reducer_sorted_device)
        ]
    ...
    def mapper_get_device(self, _, line):
    ...
    def reducer_sum_device(self, key, values):
        #yield (sum(values), key)
        yield sum(values), key


    def reducer_sorted_device(self, count, devices):
